...
function BindToID(id, img_folder) {
    console.log(id, img_folder);
    let filename = `${config.upload}/binds.marki`;
    let map = new Map();
    map.set(id, img_folder);
    console.log(JSON.stringify(map) + " is map");
    fs.writeFile(filename, JSON.stringify(map), {
        encoding: "utf-8",
        mode: 0o666,
        flag: "w"
    }, (err) => {
        if (err && err.code == "EEXIST") {
            fs.readFile(filename, (err, data) => {
                if (err) {
                    ...
                    //data.toString("utf-8");
                    let map = data.toJSON();
                    console.log(map.data + " is bindes");

                    console.log(data.toString("utf-8"));
                    map.set(id, img_folder);
                    console.log(JSON.stringify(map) + " is map");
                    fs.writeFile(filename, map, {
                        encoding: "utf-8",
                        mode: 0o666,
                        flag: "wx"
                    }, (err) => {
