...
router.get("/get-hot", upload.array(), (req, res, next) => {
    ...
    let result = public_items_managment.getNewOrHotItems(true, false);
...
router.get("/get-new", upload.array(), (req, res, next) => {
    let result = public_items_managment.getNewOrHotItems(false, true);
    
    //let hots;
    result.then((vals) => {
        console.log(JSON.stringify(vals) + " is vals");
        //console.log(vals.filter(public_items_utils.FilledArray) + " is vals");
...
router.get("/:category", upload.array(), (req, res, next) => {
    let result = public_items_managment.getCategoryItems(req.params.category);
    result.then((fulfilled) => {
        //res.send(fulfilled);
        res.render("./public/category_items", {
